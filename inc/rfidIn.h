...
#define RFID_IN_STACK_SIZE                   2048
#define RFID_IN_PRIORITY                     8
...
#define PN532_RESET_PIN (3)  // Not connected by default on the NFC Shield
...
//   Adafruit_PN532(uint8_t ss, SPIClass *theSPI = &SPI); // Hardware SPI
  Adafruit_PN532(const struct i2c_dt_spec i2cDev, struct k_msgq *_queue, const struct gpio_dt_spec irq, const struct gpio_dt_spec reset);              // Hardware I2C
//   Adafruit_PN532(uint8_t reset, HardwareSerial *theSer); // Hardware UART
  ...
  const struct gpio_dt_spec *_reset;
