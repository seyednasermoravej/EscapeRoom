...
static int i2c_dw_runtime_configure(const struct device *dev, uint32_t config)
{
    ...
    /* افزودن تنظیمات SDA Hold Time */
    uint32_t sda_hold_value = 0x00400040; // مقدار پیشنهادی برای زمان نگه‌داری (دو برابر پیش‌فرض)
    
	sys_write32(sda_hold_value, reg_base + IC_SDA_HOLD);
