...
static char queryAddress[128] = {0};
static char serverName[128] = {0};
		   ...
		   void *user_data)
{
	...
	LOG_INF("%s %s address: %s", user_data ? (char *)user_data : "<null>",
        ...
        strcpy(queryAddress, net_addr_ntop(info->ai_family, addr,
					 hr_addr, sizeof(hr_addr)));
}
...
{
	static const char *query = (const char*)serverName;
