...
LOG_MODULE_REGISTER(tcs, LOG_LEVEL_INF);
/*!
...
uint8_t Adafruit_TCS34725::read8(uint8_t reg) {
  ...
  i2c_write_read_dt(i2c_dev, buffer, 1, buffer, 1);
  return buffer[0];
}
...
uint16_t Adafruit_TCS34725::read16(uint8_t reg) {
  ...
  i2c_write_read_dt(i2c_dev, buffer, 1, buffer, 2);
...
void Adafruit_TCS34725::getRGB(float *r, float *g, float *b) {
  ...
  *r = (float)red / sum * (float)255.0;
  *g = (float)green / sum * (float)255.0;
  *b = (float)blue / sum * (float)255.0;
}
